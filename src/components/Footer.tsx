        ...
        <div className="glass-lg rounded-3xl p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
              <img src={logoDark} alt="Логотип magister1o" width="40" height="40" loading="lazy" decoding="async" className="w-10 h-10 rounded-2xl" />
              <div>
            ...
            {/* Social links */}
            <nav aria-label="Социальные профили" className="flex items-center gap-2">
              {[
              ...
              ].map(({ href, icon: Icon, label, testId, disabled }) => disabled ? (
                <span
                  key={testId}
                  title={`${label} недоступен`}
                  data-testid={testId}
                  aria-label={`${label} недоступен`}
                  className="p-2.5 rounded-2xl text-gray-300 dark:text-gray-600 cursor-not-allowed transition-all duration-200"
                >
          ...
          <div className="mt-8 pt-6 border-t border-white/50 dark:border-white/10 text-center">
            ...
            <nav aria-label="Юридическая информация" className="mt-3 flex items-center justify-center gap-4">
              <a
                href="/terms"
